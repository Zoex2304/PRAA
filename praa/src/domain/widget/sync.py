...
from src.domain.widget.time_provider import ITimeProvider
...
class SyncController:
    ...
    def load_boundaries(self, chunk_idx: int, boundaries: list[tuple]) -> None:
        ...
        try:
            ...
            if chunk_idx in self._pending_chunks:
                ...
                logger.debug(
                    "Boundaries loaded for chunk %d (%d words)", chunk_idx, len(parsed)
                )
    ...
    def _find_word_at_time(self, chunk_idx: int, position_ms: float) -> int | None:
        """
    ...
    def _apply_highlight(self, chunk_idx: int, word_idx: int) -> None:
        ...
        self._highlight(
            chunk_idx, word.text_offset, word.text_offset + word.text_length
        )
...
class SyncDiagnostics:
    ...
    def __init__(
        self, controller: SyncController, time_provider: ITimeProvider
    ) -> None:
